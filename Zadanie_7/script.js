...
// 1) Wyswietl, informacje o biurze w Gliwicach (lokalizacja, liczba przypisanych pracowników, srednia pensja),
    ...
    const infoOffice = office => company.offices.map (office =>{
        return {
    ...
    console.log(infoOffice('Gliwice'));
...
// 3) Dodaj nowego pracownika do biura w Poznaniu: { id: 16, name: "Olek", type: "M", office: "PO", salary: 500 }
    ...
    const officePoznan = company.offices.find((office) => office.id === 'PO');

    officePoznan.workers.push(newWorker);

    console.log(company.offices);

// 4) Wyswietl, informacje o biurze w Poznaniu

    console.log(officePoznan);

// 5) Wyswietl srednia pensje w calej firmie,

    const allWorkers = company.offices.reduce((acc, next) => acc.concat(next.workers),[]);

    const avarageSalarInCompany = (allWorkers.reduce((acc, next) => acc + next.salary,0/allWorkers.length));

        console.log(avarageSalarInCompany);

//  6) Wyswietl najlepiej oplacanego pracownika w poszczególnych biurach,

    const getBestPaidWorkersByOffices = () => {
        return company.offices
            .map(office => {
                const salaries = office.workers.map(worker => worker.salary);
                const highestSalary = Math.max(...salaries);
                return office.workers.filter(worker => worker.salary === highestSalary)
            })
            .reduce((acc, next) => acc.concat(next, []));
    };

    const bestPaidWorkers = getBestPaidWorkersByOffices();
    console.log(bestPaidWorkers);
...
//  7) Wyswietl najlepiej oplacanego pracownika w calej firmie oraz nazwe jego biura.
    ...
    const bestSalaryWorker = allWorkers.find((office) => office.salary === maxSalary);

    console.log(bestSalaryWorker);
